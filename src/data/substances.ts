...
export const getSubstancesByPriceRange = (
  min: number,
  max: number
): Substance[] => {
  return Object.values(SUBSTANCES).filter(
    (substance) => substance.cost >= min && substance.cost <= max
  );
};

export const getCheapestSubstances = (count: number = 5): Substance[] => {
